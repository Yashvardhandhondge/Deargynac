...
export default function ErrorBoundary({ error, reset }: { error: Error; reset: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
      ...
      <h2 className="text-xl font-bold text-[#3D3438] font-serif">Something went wrong</h2>
      <p className="text-gray-500 text-sm mt-2 max-w-md">{error.message || "An unexpected error occurred."}</p>
      <div className="flex gap-3 mt-6">
        <button onClick={reset} className="bg-[#D97894] text-white rounded-full px-6 py-2 text-sm font-semibold hover:bg-[#C45F7E] transition-colors">Try Again</button>
