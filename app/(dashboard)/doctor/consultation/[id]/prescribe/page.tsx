  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="bg-white rounded-2xl p-6 border border-gray-100">
        <h2 className="text-2xl font-bold text-[#3D3438] font-serif">Issue Prescription</h2>
        <div className="flex items-center gap-3 mt-4">
          ...
          <div>
            <p className="font-semibold text-sm text-[#3D3438]">{patientName}</p>
      ...
      <div className="bg-white rounded-2xl p-6 border border-gray-100 mt-6">
        <h3 className="font-bold text-[#3D3438] mb-4">Medications</h3>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#D97894] focus:border-[#D97894] outline-none"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#D97894] focus:border-[#D97894] outline-none bg-white"
                >
                  {frequencies.map((f) => (
        ...
        <button
          ...
          className="w-full border-2 border-dashed border-gray-300 rounded-xl py-3 text-sm font-medium text-gray-500 hover:border-[#D97894] hover:text-[#D97894] transition-colors flex items-center justify-center gap-1 disabled:opacity-50"
        >
      ...
      <div className="bg-white rounded-2xl p-6 border border-gray-100 mt-6 space-y-4">
        <div>
          ...
          <textarea
            ...
            className="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#D97894] focus:border-[#D97894] outline-none resize-none"
          />
        </div>
        <div>
      ...
      <div className="bg-[#FFF7F9] border border-rose-100 rounded-2xl p-6 mt-6">
        ...
        <div className="bg-white rounded-xl p-6 text-sm space-y-3 border border-gray-100">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs text-gray-500">
            ...
            <div>Date: <span className="text-[#3D3438] font-medium">{new Date().toLocaleDateString("en-IN")}</span></div>
            <div>Doctor: <span className="text-[#3D3438] font-medium">Dr. {doctorName}</span></div>
            <div>ID: <span className="text-[#3D3438] font-medium">{id?.toString().slice(-8)}</span></div>
          </div>
              ...
              <tbody>
                {medicines.filter((m) => m.name.trim()).map((m, i) => (
                  <tr key={i} className="border-b border-gray-50">
                    <td className="py-1.5 font-medium text-[#3D3438]">{m.name}</td>
          ...
          {notes && (
            <div className="mt-2">
              ...
              <p className="text-sm text-[#3D3438]">{notes}</p>
            </div>
          )}
          {followUpDate && (
            <div>
              <span className="text-xs text-gray-400">Follow-up:</span>{" "}
              <span className="text-sm text-[#3D3438]">{new Date(followUpDate).toLocaleDateString("en-IN")}</span>
            </div>
          )}
      ...
      <button
        ...
        disabled={isLoading}
        className="w-full bg-[#D97894] text-white rounded-full py-4 text-base font-semibold hover:bg-[#C45F7E] transition-colors mt-6 mb-8 disabled:opacity-50 flex items-center justify-center gap-2"
      >
        {isLoading ? (
